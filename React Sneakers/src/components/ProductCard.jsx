...
const ProductCard = (/* {name, price, img} */props) => {
    ...
    return (
        ...
        hover:shadow-2xl transition-all duration-200 hover:-translate-y-2 ease-in-out '>
            ...
            <div className='flex items-center justify-between w-full'>
                ...
                <button className='border border-gray-200 w-9 h-9 flex items-center justify-center rounded-lg
                cursor-pointer' onClick={props.onClick} 
                //onClick={OnClickAlert}
                // если через onCLick нужно передать аргумент то пишем () => {OnClickAlert(аргумент)}
                ><FaPlus color='lightgray' /></button>
            </div>
        </div>
    )
}
